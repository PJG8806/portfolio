...
금융 서비스는 단순 CRUD보다 권한·보안 설계가 더 중요하다는 점을 실감했습니다. 도메인 규칙을 Serializer와 DB Constraint로 이중 검증하는 방식, 부모/자녀 관계 기반 조회 정책 설계, 그리고 API 설계 단계(PRD/FlowChart/ERD)의 중요성을 직접 경험하며 배울 수 있었습니다.

또한 실제 금융 서비스 수준을 목표로 외부 API 연동까지 시도하며 기술적 확장 가능성을 고민한 경험도 의미 있었습니다.

개선할 점으로는 초기 단계에서 현실적 제약과 도메인 예외 상황을 더 충분히 정의했어야 했고, 팀원 간 코드 공유와 검증 기준을 맞추는 협업 역량이 부족했다는 점을 느꼈습니다.
